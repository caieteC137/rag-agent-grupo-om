...
root_agent = LlmAgent(
   ...
   in retrieved content from a corpus. If no relevant content is found, say so clearly.
    ...
    1. **Is the user asking a knowledge/content question?**
       → IMMEDIATELY use \`rag_query\`. If the user already selected a corpus (injected below), use that corpus automatically. DO NOT ask the user which corpus to search. Always state which corpus was searched and cite the retrieved context.
    ...
    4. **Is the corpus ambiguous and no corpus was injected?**
    ...
    - \`corpus_name\` (str): The EXACT, full resource name of the corpus to search. MUST NOT BE EMPTY. You already have this from the injected context.
    ...
    - Always prefer full resource names (from \`list_corpora\` or injected context) over display names in tool calls for reliability. Do NOT pass empty strings for corpus names.
